...
import { Urgency } from "../../../components/editor/fields";
import { TestingProvider, defaultFormData } from "../helpers";

const props = { alertData: { ...defaultFormData, urgency: "" } };

describe("<Urgency>", () => {
  test("renders correctly", async () => {
    const onUpdate = jest.fn();
    render(<Urgency {...props} onUpdate={onUpdate} />, {
  ...
  test("triggers callback on option click correctly", async () => {
    ...
    const option = await screen.findByText("Immediate");
    await user.click(option);
    expect(onUpdate).toBeCalledTimes(1);
    expect(onUpdate).toBeCalledWith({ urgency: "Immediate" });
  });
});
